...
from ddms.db.models import Device, Group, Role
...
@router.post("/", response_model=GroupOut, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_roles(Role.ADMIN, Role.OWNER))])
...
@router.patch("/{group_id}", response_model=GroupOut, dependencies=[Depends(require_roles(Role.ADMIN, Role.OWNER))])
def groups_rename(payload: GroupUpdate, session: SessionDep, group_id: int = Path(..., ge=1)) -> GroupOut:
...
@router.post("/{group_id}/devices/{device_id}", dependencies=[Depends(require_roles(Role.ADMIN, Role.OWNER))])
def groups_assign(session: SessionDep, group_id: int = Path(..., ge=1), device_id: int = Path(..., ge=1)) -> dict[str, bool]:
    g = session.get(Group, group_id)
    if not g:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Group not found")
...
@router.delete("/{group_id}/devices/{device_id}", dependencies=[Depends(require_roles(Role.ADMIN, Role.OWNER))])
def groups_unassign(session: SessionDep, group_id: int = Path(..., ge=1), device_id: int = Path(..., ge=1)) -> dict[str, bool]:
    g = session.get(Group, group_id)
    if not g:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Group not found")
    d = get_device(session, device_id)
