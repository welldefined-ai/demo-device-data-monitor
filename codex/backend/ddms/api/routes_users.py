...
from ddms.api.deps import SessionDep, UserDep, require_roles
from ddms.core.security import hash_password
from ddms.db.models import Role, User
from ddms.db.repositories.users import (
    create_user,
    delete_user,
    get_by_username,
    list_users,
...
def users_create(payload: UserCreate, session: SessionDep) -> UserOut:
    if payload.role == Role.OWNER:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot create owner via API"
        )
...
def users_update(
    ...
    if (
        new_username
        and new_username != target.username
        and get_by_username(session, new_username)
    ):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Username already exists"
        )
...
def users_delete(session: SessionDep, user_id: int = Path(..., ge=1)) -> dict[str, bool]:
    ...
    if target.role == Role.OWNER:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot delete owner account"
        )
