...
def _parse_modbus_number(registers: list[int], data_type: str = "uint16", byteorder: str = "big") -> float:
...
def _read_modbus(cfg: dict[str, Any]) -> float:
    ...
    """
    from pymodbus.client import ModbusSerialClient, ModbusTcpClient  # lazy import

    typ = str(cfg.get("type", "tcp")).lower()
    data_type = str(cfg.get("data_type", "uint16")).lower()
    byteorder = str(cfg.get("byteorder", "big")).lower()
    ...
    try:
        ...
        rr = client.read_holding_registers(address=register, count=count, slave=unit)
        if rr.isError():  # type: ignore[attr-defined]
            raise ConnectionError(str(rr))
        regs: list[int] = list(rr.registers)  # type: ignore[attr-defined]
        return _parse_modbus_number(regs, data_type=data_type, byteorder=byteorder)
    finally:
        try:
            client.close()
